...
console.log("lib/pgbuild/create");
    ...
    // app name
    fs.readFile(path.join('config.xml'), 'utf8', function(e, data) {
        if (e) {
            callback(e);
            return;
        }
        ...
        zip.compress(paths.www, paths.build, function(e, filename) {
            ...
            options.api.post(url, headers, function(e, response) {
                ...
                // update config.json
                config.local.load(function(e, configdata) {
                    if (e) {
                        callback(e);
                        return;
                    }
                    ...
                    configdata.phonegap = configdata.phonegap || {};
                    configdata.phonegap.id = response.id;
                    
                    config.local.save(configdata, function(e) {
                        if (e) {
                            callback(e);
                            return;
                        }
